"""Source node runtime handler - primary media source for processing."""
...
class SourceNodeHandler:
    """Handles the Source node - primary media source for processing.
    
    This node explicitly designates the main material to be processed.
    Unlike Load nodes (which are auxiliary), Source is the reference
    used for splitter comparison and processing.
    """
    key = "source"
    ...
    def execute(
        ...
        if not media_path:
            raise NodeExecutionError("Source: path is empty")
        if not os.path.exists(media_path):
            raise NodeExecutionError(f"Source: file not found: {media_path}")
        ...
        context.state["source_media"] = payload
        return {"out": payload}
