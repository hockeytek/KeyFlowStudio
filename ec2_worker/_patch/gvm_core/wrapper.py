...
class GVMProcessor:
    ...
    def process_sequence(self, input_path, output_dir, 
        ...
        for batch_id, batch in tqdm(enumerate(dataloader), total=len(dataloader), desc=f"Inferencing {file_name}"):
            ...
            # Upscale back to original input resolution (GVM internally scales
            # to 1024px short side / 1920px long side, but output must match input).
            if (orig_h, orig_w) != current_upscaled_shape:
                alpha = F.interpolate(alpha, (orig_h, orig_w), mode='bilinear', align_corners=False)
