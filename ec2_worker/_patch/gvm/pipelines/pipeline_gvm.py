...
class GVMPipeline(DiffusionPipeline, GVMLoraLoader):
    ...
    def encode(self, input, encode_chunk_size=1):
        num_frames = input.shape[1]
        input = input.flatten(0, 1)  # [batch*frames, C, H, W]

        import gc as _gc

        # Offload UNet to CPU before VAE encode to free ~7-8 GiB on A10G.
        # VAE encode needs ~3 GiB per 1024×1920 frame; after UNet forward pass
        # the allocator has <2 GiB free despite 4+ GiB reserved (fragmentation).
        # Moving UNet off GPU + empty_cache() reclaims that fragmented space.
        cuda_device = None
        if input.device.type == "cuda" and hasattr(self, "unet"):
            cuda_device = next(self.unet.parameters()).device
            self.unet.to("cpu")
            _gc.collect()
        ...
        for i in range(0, input.shape[0], encode_chunk_size):
            ...
            latents.append(self.vae.encode(chunk).latent_dist.mode())
        latent = torch.cat(latents, dim=0)

        # Restore UNet to GPU before UNet inference steps
        if cuda_device is not None:
            self.unet.to(cuda_device)
